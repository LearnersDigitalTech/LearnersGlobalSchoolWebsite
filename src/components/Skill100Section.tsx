...
// ─────────────────────────────────────────────
//  Skill100Section — Learning Centres
//  Place this file at: src/components/Skill100Section.tsx
//  Place CSS at:       src/components/Skill100Section.module.css
// ─────────────────────────────────────────────
...
interface Campus {
  ...
  src: string
  alt: string
  badge: { text: string; green: boolean } | null
  title: string
  location: string
  cta: string
}
          ...
          {campuses.map((campus) => (
            ...
            >
              <div className={styles.imgWrap}>
                {/* Using <img> instead of next/image to avoid standalone/Cloud Run issues */}
                <img
                  src={campus.src}
                  alt={campus.alt}
                ...
                {campus.badge && (
                  <span
                    className={
                      campus.badge.green
                        ? `${styles.campusBadge} ${styles.badgeGreen}`
                        : styles.campusBadge
                    }
                  >
                    {campus.badge.text}
